...
corpus = os.path.join("conel", corpus_name)
...
def batch2TrainData(voc, pair_batch):
    pairs = pair_batch.sort(key=lambda x: len(x[0].split(" ")), reverse=True)
...
def batch2TrainDatao(voc, pair_batch):
...
def batch2TrainData(voc,pair_batch):
    ...
    inp, lengths = inputVar(inputlist,voc)
    output, mask, max_target_len = outputVar(outputlist,voc)
    return inp,lengths,output,mask,max_target_len
...
class EncoderRNNone(nn.Module):
    def __init__(self, hidden_size, embedding, n_layers = 1, dropout = 0):
        super(EncoderRNNone,self).__init__()
        self.hidden_size = hidden_size
        self.n_layers = n_layers
        self.embedding = embedding
...
class Attnone(torch.nn.Module):
    def __init__(self, method, hidden_size):
        super(Attnone, self).__init__()
        self.method = method
        if self.method not in ['dot','general','concat']:
            raise ValueError(self.method, "is not a appropriate attention method")
        self.hidden_size = hidden_size
        if self.method == "general":
            self.attn = torch.nn.Linear(self.hidden_size,hidden_size)
        elif self.method == "concat":
            self.attn = torch.nn.Linear(self.hidden_size*2,hidden_size)
            self.v = torch.nn.parameters(torch.FloatTensor(hidden_size))

    def dot_score(self, hidden, encoder_output):
        """
        function: get attention by dot
        hidden: hidden states | a tensor with shape B*h
        encoder_outputs: outputs of encoder | a tensor with shape L*B*h 
        return a tensor with shape L * B
        """
        return torch.sum(hidden * encoder_output, dim = 2)
    ...
    def general_score(self, hidden, encoder_output):
        """
        function: get attention by matrix
        hidden: a tensor with shape B*h
        encoder_output: a tensor with shape L*B*h
        return a tensor with shape L*B
        """
        return torch.sum(hidden * self.attn(encoder_output), dim = 2)

    
    def concat_score(self,hidden,encoder_output):
        """
        function: get attention by concat
        hidden: a tensor with shape B*h
        encoder_outputs: a tensor with shape L*B*h
        return a tensor with shape L*B
        """
        energy = self.attn(torch.cat(hidden.expand((encoder_output.size(0),-1,-1),encoder_output),dim = 2))
        return torch.sum(self.v * energy, dim = 2)

    def forward(self, hidden, encoder_outputs):

        """
        function: get present attention
        hidden: a tensor with shape B*h
        encoder_outputs: a tensor with shape L*B*h
        return: attention weight of encoder_outputs | a tensor with shape B*L
        """
        if self.method == "dot":
            attn_energies = self.dot_score(hidden,encoder_outputs)
...
class LuongAttnDecoderRNN(nn.Module):
    def __init__(self, attn_model, embedding, hidden_size, output_size, n_layers=1, dropout=0.1):
        ...
        self.attn = Attnone(attn_model, hidden_size)
...
class LuongAttnDecoderRNNone(nn.Module):
    def __init__(self,attn_model, embedding, hidden_size, output_size, n_layers = 1, dropout=0.1):
        super(LuongAttnDecoderRNNone,self).__init__()
        self.atten_model = attn_model
        self.hidden_size = hidden_size
        self.output_size = output_size
        self.n_layers = n_layers
        self.dropout = dropout
...
# mask tensor.
#
def maskNLLLossone(inp,target,mask):
    ntotal = mask.sum()
...
def train(input_variable, lengths, target_variable, mask, max_target_len, encoder, decoder, embedding,
    ...
    if use_teacher_forcing:
        for t in range(max_target_len):
            ...
            mask_loss, nTotal = maskNLLLossone(decoder_output, target_variable[t], mask[t])
            loss += mask_loss
...
def train_one(input_variable, lengths, target_variable, mask, max_target_len, encoder, decoder, embedding, encoder_optimizer, decoder_optimizer, batch_size, clip, max_length=MAX_LENGTH):
    # zero the optimer
    encoder_optimizer.zero_grad()
    decoder_optimizer.zero_grad()
    # put variable to device
    input_variable = input_variable.to(device)
    lengths = lengths.to(device)
    mask = mask.to(device)
    target_variable = target_variable.to(device)
    ...
    if use_teacher_forcing:
        for t in range(max_target_len):
            ...
            n_totals = n_totals + nTotal
    else:
        for t in range(max_target_len):
            decoder_output, decoder_hidden = decoder(decoder_input,decoder_hidden,encoder_output)
            _,tops = decoder_output.topk(1)
            decoder_input = torch.LongTensor([[tops[0][i] for i in range(batch_size)]])
            mask_loss, nTotal = maskNLLLossone(decoder_output,target_variable[t],mask[t])
            loss = loss + mask_loss
            print_loss.append(mask_loss.item() * nTotal)
            n_totals = n_totals + nTotal
...
encoder = EncoderRNNone(hidden_size, embedding, encoder_n_layers, dropout)
decoder = LuongAttnDecoderRNNone(attn_model, embedding, hidden_size, voc.num_words, decoder_n_layers, dropout)
if loadFilename:
